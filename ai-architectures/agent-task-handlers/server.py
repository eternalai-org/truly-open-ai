...
def _setup_scheduling():
    ...
    def start_non_blocking_io_scan_and_resume_task(once=False) -> Optional[schedule.CancelJob]:
        global previous_non_blocking_io_resume_task

        if previous_non_blocking_io_resume_task is None or not previous_non_blocking_io_resume_task.is_alive():
    ...
    schedule.every(10).minutes.do(lambda: start_non_blocking_io_scan_and_resume_task(once=True))
    schedule.every(30).minutes.do(lambda: start_non_blocking_io_scan_and_resume_task(once=False))
...
if __name__ == "__main__":
    ...
    _setup_scheduling(); scheduler_thread.start()
    uvicorn.run(
        app=app,
