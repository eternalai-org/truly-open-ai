...
class ConversationRedisCache(object):
    ...
    def is_threshold_exceeded(self, username, root_conversation_id, max_num_tweets_in_conversation=1):
        mentioned_tweets_redis_key = self._get_key(username)
        if not self.redis_client.exists(mentioned_tweets_redis_key):
            return False
        conversation_dict = json.loads(self.redis_client.get(mentioned_tweets_redis_key))
        cached_tweets_by_root_conversation = conversation_dict.get(root_conversation_id, [])
        return len(cached_tweets_by_root_conversation) >= max_num_tweets_in_conversation

    @log_function_call
    def add_tweet_to_conversation(self, username, root_conversation_id, tweet_object):
        mentioned_tweets_redis_key = self._get_key(username)
        if not self.redis_client.exists(mentioned_tweets_redis_key):
            self.redis_client.set(mentioned_tweets_redis_key, json.dumps({}), ex=self.cache_expiry)
        conversation_dict = json.loads(self.redis_client.get(mentioned_tweets_redis_key))

        cached_tweets_by_root_conversation = conversation_dict.get(root_conversation_id, [])
        cached_tweets_by_root_conversation.append(tweet_object)
        conversation_dict[root_conversation_id] = cached_tweets_by_root_conversation
        self.redis_client.set(mentioned_tweets_redis_key, json.dumps(conversation_dict), ex=self.cache_expiry)
...
class GameRedisCache(object):
    ...
    def add_running_game(self, tweet_id: str, status: str) -> bool:
        try:
            with self.redis_client.pipeline() as pipe:
                pipe.multi()
                pipe.set(f"{self.GAMES_STATUS_PREFIX}{tweet_id}", status, ex=self.cache_expiry)
    ...
    def acquire_create_game_lock(self, _tweet_id, expiry_ms: int) -> bool:
        return self._acquire_lock(f"{self.GAMES_LOCK_PREFIX}create/{_tweet_id}", expiry_ms)
    
    def acquire_judge_game_lock(self, _tweet_id, expiry_ms: int) -> bool:
        return self._acquire_lock(f"{self.GAMES_LOCK_PREFIX}judge/{_tweet_id}", expiry_ms)
...
class ShadowReplyRedisCache(object):
    ...
    def is_threshold_exceeded(self, username, tweet_id, max_num_reply=1):
        ...
        if not self.redis_client.exists(shadow_replied_redis_key):
            return False
        shadow_reply_dict = json.loads(self.redis_client.get(shadow_replied_redis_key))
    ...
    def add_reply(self, username, tweet_id):
        shadow_replied_redis_key = self._get_key(username)
        if not self.redis_client.exists(shadow_replied_redis_key):
            self.redis_client.set(shadow_replied_redis_key, json.dumps({}), ex=self.cache_expiry)
        shadow_reply_dict = json.loads(self.redis_client.get(shadow_replied_redis_key))

        replies_count = shadow_reply_dict.get(tweet_id, 0)
        shadow_reply_dict[tweet_id] = replies_count + 1
        self.redis_client.set(shadow_replied_redis_key, json.dumps(shadow_reply_dict), ex=self.cache_expiry)
...
class TweetInfoRedisCache(object):
    ...
    def commit(self, tweet_info):
        ...
        self.redis_client.set(key, json.dumps(tweet_info), ex=self.cache_expiry)
...
class TweetInscriptionRedisCache(object):
    ...
    def add_inscription(self, username, request_id, tweet_id):
        ...
        if not self.redis_client.exists(inscribe_redis_key):
            self.redis_client.set(inscribe_redis_key, json.dumps({}), ex=self.cache_expiry)
        ...
        self.redis_client.set(inscribe_redis_key, json.dumps(inscribe_dict), ex=self.cache_expiry)
    ...
    def get_inscribed_tweets_ids(self, username):
        ...
        inscribe_dict: dict[str, list] = json.loads(self.redis_client.get(inscribe_redis_key))
