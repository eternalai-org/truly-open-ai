...
class ReactAgentReasoningMeta(BaseModel):
    ...
    system_reminder: Optional[str] = None # set a reminder for to LLM for each step
    params: Dict[str, Any] = { 
    ...
    def validate_kb_agents(cls, data: dict):
        ...
        if isinstance(data['kb_agents'], str):
            try:
                ...
                logger.warning(f"Failed to parse kb_agents: {e}; Received: {data['kb_agents']}")
                data['kb_agents'] = []

        if isinstance(data['kb_agents'], list):
            data['kb_agents'] = [AgentKnowledgeBase(**x) for x in data['kb_agents']]
...
def modified_json_stringnify(obj: dict, depth=0, indent=2) -> str:
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            res.append('{k}: {v}'.format(leading_space, k=k, v=modified_json_stringnify(v, depth + 1, indent)))
    ...
    def validate_default(cls, data: dict):
        ...
        if 'default_value' in data and data['default_value'] is not None:
            if data['dtype'] == ToolParamDtype.BOOLEAN:
                data['default_value'] = data['default_value'] in ['true', 'True', '1', 'TRUE', True]

            if data['dtype'] == ToolParamDtype.NUMBER:
                try:
                    data['default_value'] = float(data['default_value'])
                except Exception as e:
                    raise ValueError(f"Invalid default_value value for number type: {data['default_value']}")
...
class ToolDef(BaseModel):
    ...
    def prototype(self):
        no_default_params = [param for param in self.params if param.default_value is None]
...
class ReasoningLog(BaseModel):
    ...
    def _system_prompt_(self) -> str:
        return modified_json_stringnify(self.agent_meta_data.json_system_prompt())
    ...
    created_at: str = Field(default_factory=lambda: datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%fZ"))
    last_process: str = Field(default_factory=lambda: datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%fZ"))
    ...
    def forecast_agent_meta_data(self):
        ...
        if (self.agent_meta_data.persona is None or self.agent_meta_data.persona == "") and self.system_prompt is not None:
    ...
    def parse_tool_list(cls, data: dict):
        ...
        if 'tool_list' not in data or data['tool_list'] == None:
            ...
            if isinstance(data['tool_list'], list):
                data['tool_list'] = [AdvanceToolDef(**x) for x in data['tool_list']]

        if 'params' in data.get('meta_data', {}):
            if 'react_max_steps' in data['meta_data']['params']:
                data['meta_data']['params']['react_max_steps'] = int(data['meta_data']['params']['react_max_steps'])

            else:
                data['meta_data']['params']['react_max_steps'] = const.DEFAULT_REACT_MAX_STEPS
    ...
    def checksum(self):
        ...
        try:
            ...
            logger.error(f"Failed to encode the reasoning log to get checksum: {err}! Returning a non-standard stringified json to encode instead.")
...
class TwinUpdateResponse(BaseModel):
    ...
    twin_training_progress: int = Field(..., description="Twin training progress")
