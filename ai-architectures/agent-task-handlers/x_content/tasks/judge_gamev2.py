...
from x_content.wrappers.api.twitter_v2.models.response import ExtendedTweetInfoDto, GenerateActionDto, Response, TweetsDto
from x_content.wrappers.game import GameAPIClient, GameInfo, _get_game_redis_cache, GameStatus, GameState

from x_content.tasks.utils import a_move_state, create_twitter_auth_from_reasoning_log

from x_content.wrappers.junk_tasks import get_image_description
...
async def _get_ready_to_judge_games():
    ...
    try:
        ...
        logger.info("[_get_ready_to_judge_games] Getting running games from Redis")
        ...
        logger.info(f"[_get_ready_to_judge_games] Found {len(running_games)} running games, checking status...")
        
        # Get game info for all running games in one API call
        games_info, err = await sync2async(GameAPIClient.get_game_info_by_tweet_ids)(running_games)
        
        if err:
            logger.error(f"[_get_ready_to_judge_games] Error getting game info: {err}")
            return []
        ...
        for game in games_info:
            if game.status == GameState.ENDED:
                logger.info(f"[_get_ready_to_judge_games] Game {game.tweet_id} is ready to judge")
                ready_tweet_ids.append(game.tweet_id)

        logger.info(f"[_get_ready_to_judge_games] Found {len(ready_tweet_ids)} games ready to judge")
        return ready_tweet_ids

    except Exception as err:
        logger.error(f"[_get_ready_to_judge_games] Error getting ready games: {err}")
        return []
    
def _is_judge_game_pending(tweet_id):
    game_redis = _get_game_redis_cache()
    status = game_redis.get_game_status(tweet_id)
    is_pending = status == GameStatus.CREATE_DONE or status == GameStatus.JUDGE_PENDING
    logger.info(f"[_is_judge_game_pending] Game {tweet_id} status: {status} {status == GameStatus.CREATE_DONE } pending status: {is_pending}")
    return is_pending

def _mark_judge_game_pending(tweet_id):
    logger.info(f"[_mark_judge_game_pending] Marking game {tweet_id} as pending")
    game_redis = _get_game_redis_cache()
    return game_redis.set_game_status(tweet_id, GameStatus.JUDGE_PENDING)

def _mark_judge_game_running(tweet_id):
    logger.info(f"[_mark_judge_game_running] Marking game {tweet_id} as running")
    game_redis = _get_game_redis_cache()
...
def _mark_judge_game_done(tweet_id):
    ...
    try:
        ...
        logger.error(f"[_mark_judge_game_done] Error marking game as done: {err}")
...
def _try_acquire_judge_game_lock(_tweet_id):
    ...
    try:
        ...
        lock = game_redis.acquire_judge_game_lock(
            _tweet_id,
            const.JUDGE_GAME_LOCK_EXPIRY  # e.g., 300 seconds
        )
        logger.info(f"[_try_acquire_judge_game_lock] Lock acquisition attempt for game {_tweet_id}: {'successful' if lock else 'failed'}")
        ...
        if not lock:  # If lock wasn't acquired
            try:
                ...
                logger.info(f"[_try_acquire_judge_game_lock] Released lock for game {_tweet_id}")
            except Exception as e:
                # Log any errors during lock release
                logger.error(f"Error releasing lock for tweet {_tweet_id}: {str(e)}")
...
async def _get_judge_game_conversation(game_tweet_object, answers):
    ...
    """
    logger.info("[_get_judge_game_conversation] Building conversation for judging")
    ...
    if game_tweet_object.get('image_urls'):
        for img_url in game_tweet_object.get('image_urls'):
            content_images += await sync2async(get_image_description)(img_url)
            content_images += "\n"
    ...
    logger.info(f"[_get_judge_game_conversation] Successfully built conversation: {conversation_thread}")
...
async def _get_child_tweets(tweet_id):
    ...
    """
    try:
        logger.info(f"[_get_child_tweets] Fetching child tweets for tweet {tweet_id}")
        # query = f"conversation_id:{tweet_id}" ##TODO:@harvey update this query 


        resp: Response[TweetsDto] = await sync2async(twitter_v2.search_recent_tweet_by_tweetid)(tweet_id)
        if resp.is_error():
            raise Exception(resp.error)
        recent_replies = resp.data.tweets

        logger.info(f"[_get_child_tweets] Found {len(recent_replies)} child tweets")
        ...
        logger.error(f"[get_child_tweets_from_api] Error getting child tweets: {err}")
        return []


async def _get_final_answers(log: ReasoningLog, _tweet_id):
    try:
        logger.info(f"[_get_final_answers] Getting final answers for game {_tweet_id}")
        # Get child tweets and game data
        child_tweets, game_data, err = await _get_game_data_and_tweets(_tweet_id)
        if err is not None:
            return None, err
            
        # Get participant answers
        participants_answers, err = _get_participant_answers_from_tweets(game_data, child_tweets)
        if err is not None:
            return None, err
        answers = _format_answers_within_time_limit(game_data,participants_answers)
        logger.info(f"[_get_final_answers] Successfully got {len(answers)} final answers")
        return answers, None
...
async def _get_game_data_and_tweets(tweet_id):
    ...
    logger.info(f"[_get_game_data_and_tweets] Getting child tweets for {tweet_id}")
    child_tweets = await _get_child_tweets(tweet_id)
    if not child_tweets:
        logger.error(f"[_get_game_data_and_tweets] No child tweets found for tweet {tweet_id}")
        return None, None, Exception("No child tweets found")
    logger.info(f"[_get_game_data_and_tweets] Found {len(child_tweets)} child tweets")
    
    logger.info(f"[_get_game_data_and_tweets] Getting game data from BE for {tweet_id}")
    game_data, err = await sync2async(GameAPIClient.get_game_info_by_tweet_id)(tweet_id)
    if err:
...
def _get_participant_answers_from_tweets(game_data: GameInfo, child_tweets):
    ...
    logger.info(f"[_get_participant_answers_from_tweets] Found {len(participants)} participants")
    ...
    for tweet in sorted_child_tweets:
        if tweet["username"] in participants and tweet["username"] not in seen_usernames:
            participants_answers.append(tweet)
            seen_usernames.add(tweet["username"])
            
    logger.info(f"[_get_participant_answers_from_tweets] Found {len(participants_answers)} participant answers")
    return participants_answers, None

async def _handle_winner_without_llm(log: ReasoningLog, game_id, winner_agent: str):
    """Handle case when there is no partipipant or only one participant, making them the winner by default"""
    logger.info(f"[_handle_winner_without_llm] Posting game result without calling LLM, winner if any: {winner_agent}")
...
async def _handle_winner_with_llm(log: ReasoningLog, llm, game_id, answers):
    ...
    resp: Response[ExtendedTweetInfoDto] = await sync2async(twitter_v2.get_tweet_info_from_tweet_id)(game_id, True)
    if resp.is_error():
        return None, resp.error

    tweet_obj = resp.data.tweet_info.to_dict()
    conversation_thread = await _get_judge_game_conversation(tweet_obj, answers)
    
    logger.info("[_handle_winner_with_llm] Calling LLM for judgment")
    infer_result: OnchainInferResult = await llm.agenerate(conversation_thread, temperature=0.7)
    
    winning_agent, err = _get_winner_from(infer_result.generations[0].message.content)      
    if err is not None:
        logger.error(f"[_handle_winner_with_llm] Error getting winner from LLM: {err}")
        return None, err
    
    logger.info(f"[_handle_winner_with_llm] LLM selected winner: {winning_agent}")
    _, err = await _post_game_result(log, game_id, winning_agent)
    if err is not None:
        logger.error(f"[_handle_winner_with_llm] Error posting game result: {err}")
        return None, err
    
    return winning_agent, infer_result.tx_hash, None


def _format_answers_within_time_limit(game_data: GameInfo, participants_answers):
    """Format answers that were submitted within time limit"""
    if not participants_answers:
        logger.info("[_format_answers_within_time_limit] No participant answers found")
        return []
    participants_answers_within_time_limit = [
        participant_answer for participant_answer 
        in participants_answers 
        if participant_answer["posted_at"] <= game_data.end_time
    ]
    
    logger.info(f"[_format_answers_within_time_limit] Found {len(participants_answers_within_time_limit)} answers within time limit")
...
def _get_winner_from(result: str):
    ...
    """
    try:
        logger.info("[_get_winner_from] Parsing LLM result to determine winner")
...
async def _post_game_result(log: ReasoningLog, game_id, winning_agent):
    ...
    """
    logger.info(f"[_post_game_result] Posting game result for {game_id}, winner: {winning_agent if winning_agent else 'No winner'}")
    
    # Call API to record game result
    _, err = await sync2async(GameAPIClient.submit_game_result)(game_id, winning_agent)
    if err is not None:
        logger.error(f"[_post_game_result] Error posting to API: {err}")
        return None, err

    # Post tweet announcing winner
    tweet_content = const.WINNER_TWEET_TEMPLATE.format(winning_agent) if winning_agent else const.NO_WINNER_TWEET
    logger.info(f"[_post_game_result] Posting result tweet: {tweet_content}")
    
    resp: Response[GenerateActionDto] = await sync2async(twitter_v2.reply_multi)(
...
class JudgeGameTask(MultiStepTaskBase):
    async def process_task(self, log: ReasoningLog) -> ReasoningLog:
        if log.state == MissionChainState.NEW:
            ...
            log = await a_move_state(log, MissionChainState.RUNNING, "Task started")
            log.execute_info = {
        ...
        if len(game_ids_need_to_judge) == 0:
            ...
            return await a_move_state(log, MissionChainState.DONE, "No game need to judge")
        
        for idx in range(log.execute_info["processing_idx"], len(game_ids_need_to_judge)):
            log.execute_info["processing_idx"] = idx
            game_id = game_ids_need_to_judge[idx]
            logger.info(f"[JudgeGameTask.process_task] Processing game {game_id} ({idx + 1}/{len(game_ids_need_to_judge)})")
            
            
            if not _is_judge_game_pending(game_id): 
                logger.info(f"[JudgeGameTask.process_task] Game {game_id} is not pending, skipping")
                continue
        
            if not _try_acquire_judge_game_lock(game_id ):
                logger.info(f"[JudgeGameTask.process_task] Game {game_id} is being processed by another instance, skipping...")
                continue

            # Check if tweet exists and matches game ID
            resp: Response[ExtendedTweetInfoDto] = await sync2async(twitter_v2.get_tweet_info_from_tweet_id)(game_id)
            if resp.is_error() or resp.data is None or resp.data.tweet_info is None:
                logger.info(f"[JudgeGameTask.process_task] Tweet {game_id} not found or error getting info, removing from cache")
                ...
                continue
            elif resp.data.tweet_info.tweet_object.to_dict().get("tweet_id") != game_id:
                logger.info(f"[JudgeGameTask.process_task] Tweet {game_id} does not match game ID, removing from cache")
                _mark_judge_game_done(game_id) # Remove from cache since tweet doesn't exist
                continue
            ...
            if err is not None:
                logger.error(f"[JudgeGameTask.process_task] Error getting final answers for game {game_id}: {err}")
                _mark_judge_game_pending(game_id)
                return await a_move_state(log, MissionChainState.ERROR, "Judge game failed")
            
            # Handle case with no answers or single participant
            tx_hash = None
            winning_agent = None
            if len(answers) == 0:
                logger.info(f"[JudgeGameTask.process_task] No answers for game {game_id}")
                ...
                logger.info(f"[JudgeGameTask.process_task] Single participant for game {game_id}: {winning_agent}")
                err = await _handle_winner_without_llm(log, game_id, winning_agent)
            else:
                # Create conversation for judging
                logger.info(f"[JudgeGameTask.process_task] Multiple participants for game {game_id}, using LLM")
                winning_agent, tx_hash, err = await _handle_winner_with_llm(log, self.llm, game_id, answers)
            
            if err is not None:
                logger.error(f"[JudgeGameTask.process_task] Error handling winner for game {game_id}: {err}")
                _mark_judge_game_pending(game_id)
                return await a_move_state(log, MissionChainState.ERROR, "Judge game failed")
        ...
        logger.info("[JudgeGameTask.process_task] Successfully completed all games")
        return await a_move_state(log, MissionChainState.DONE, "Judge game done")
