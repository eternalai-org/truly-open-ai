...
from .utils import format_prompt_v2
...
def dynamic_system_reminder(log: ReasoningLog, it: int, tools: List[ToolDef]) -> str:
    query_tools = [
        tool for tool in tools 
        if tool.label == ToolLabel.QUERY
    ]
    tool_name_str = ', '.join([tool.name for tool in query_tools])

    if it <= int(log.meta_data.params.get('react_max_steps', const.DEFAULT_REACT_MAX_STEPS)) * 3 // 4:
        return "Follow the instructions carefully and step-by-step do the task. Hint: use the following tools ({}) to get necessary information, news, update your context before taking any action!".format(tool_name_str)

    if len(query_tools) > 0 and not has_news(log, it, tools):
        return "Find the latest news or necessary information to the task by using one of the following tools: {}".format(tool_name_str)

    action_tools = [
        tool for tool in tools 
        if tool.label == ToolLabel.ACTION
    ]

    if len(action_tools) > 0 and not has_action(log, it, tools):        
        tool_name_str = ', '.join([tool.name for tool in action_tools])
        return "Complete the task by taking an action: {}; or provide a final_answer".format(tool_name_str)
...
def render_conversation(log: ReasoningLog, tools: List[ToolDef]):
    conversation = [
        {
            "role": "system", 
            "content": format_prompt_v2(log, tools)
        }
    ]
...
class ReactAgent(MultiStepTaskBase):
    ...
    async def is_react_complete(self, log: ReasoningLog) -> bool:
        ...
        ) or len(log.scratchpad) > log.meta_data.params.get("react_max_steps", const.DEFAULT_REACT_MAX_STEPS)
    ...
    async def update_react_scratchpad(self, log: ReasoningLog, pad: dict):
        ...
        if 'thought' in pad:
            if 'thought' in log.scratchpad[-1] and any(
                ...
                return await a_move_state(log, MissionChainState.ERROR, "Action/Action Input/Observation of the last step not found")

            log.scratchpad.append({
                "thought": pad['thought']
            })

            logger.info(f"[{log.id}][React-Iter {len(log.scratchpad)}] 🤔 Thought: {pad['thought']}")
        ...
        if 'action' in pad:
            if 'action_input' not in pad:
                ...
                return await a_move_state(log, MissionChainState.ERROR, "Action input not found")

            if 'question' in log.scratchpad[-1]:
                return await a_move_state(log, MissionChainState.ERROR, "No thought found!")

            action = pad['action']
            logger.info(f"[{log.id}][React-Iter {len(log.scratchpad)}] 🛠️ Action: {action}")

            action_input = pad['action_input']
            logger.info(f"[{log.id}][React-Iter {len(log.scratchpad)}] 📥 Action Input: {action_input}")
            ...
            for tool in tools:
                if tool.name == action:
                    ...
                    observation = await execute_tool(tool, action_input, request_id=log.id)
                    new_observation = []

                    for x in observation:
                        if isinstance(x, tuple):
                            x, metadata = x
                            log.execute_info["tool_call_metadata"].append(metadata)
            ...
            logger.info(f"[{log.id}][React-Iter {len(log.scratchpad)}] 🔍 Observation: {observation}")

        if 'final_answer' in pad:
            if any(k in log.scratchpad[-1] for k in ['action', 'action_input', 'observation']):
                log.scratchpad.append({})

            logger.info(f"[{log.id}][React-Iter {len(log.scratchpad)}] 🎯 Final Answer: {pad['final_answer']}")
            log.scratchpad[-1].update({
                "final_answer": pad['final_answer']
            })

            log = await a_move_state(log, MissionChainState.DONE, "Final answer found")


        if len(log.scratchpad) > log.meta_data.params.get('react_max_steps', const.DEFAULT_REACT_MAX_STEPS):
            log = await a_move_state(log, MissionChainState.DONE, "Scratchpad length exceeded")
