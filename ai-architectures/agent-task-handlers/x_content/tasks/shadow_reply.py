...
class ShadowReplyTask(MultiStepTaskBase):
    ...
    async def process_task(self, log):
        ...
        if log.state == MissionChainState.NEW:
            resp: Response[TweetInfosDto] = await sync2async(twitter_v2.get_full_conversation_from_liked_tweets)(
                auth=await sync2async(create_twitter_auth_from_reasoning_log)(log),
                num_tweets=5, 
                ignore_replied_tweets=True
            )
            if resp.is_error():
                return await a_move_state(log, MissionChainState.ERROR, f"Error retrieving shadow reply tweets: {resp.error}")
            ...
            return await a_move_state(log, MissionChainState.RUNNING, f"Start shadow reply task; found {len(liked_tweets)} liked tweets")
        ...
        if log.state == MissionChainState.RUNNING:
            ...
            for idx, infer in enumerate(asyncio.as_completed(futures)):
                ...
                result = await sync2async(post_process_tweet)(infer_result.generations[0].message.content)
                ...
                await sync2async(twitter_v2.shadow_reply)(
                    await sync2async(create_twitter_auth_from_reasoning_log)(log),
            ...
            return await a_move_state(log, MissionChainState.DONE, "Replied to all {} liked tweets")
