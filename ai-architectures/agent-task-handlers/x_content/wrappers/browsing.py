...
def clean_text(crawled_text: str):
    ...
    try:
        cleaned_text = repair_json(content, return_objects=True)["cleaned_text"]
        ...
        if response.status_code == 200:
            if 'application/pdf' in response.headers.get('Content-Type', ''):
                ...
                try:
                    ...
                    logger.error(f"[_crawl_data_from_url] Error processing pdf: {err}")
                    return "", []
...
def get_cleaned_text(full_text: str):
    if "http" in full_text:
        ...
        for url in urls:
            ...
            if crawled_text and len(crawled_text) >= const.MIN_TEXT_LENGTH_TO_SUMMARIZE:
                cleaned_text += clean_text(crawled_text) + "\n"

            for sub_url in sub_urls:
                sub_crawled_text, _ = _crawl_data_from_url(sub_url)
                if sub_crawled_text and len(sub_crawled_text) >= const.MIN_TEXT_LENGTH_TO_SUMMARIZE:
