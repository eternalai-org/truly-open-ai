...
class GameInfo:
    ...
    def from_dict(cls, game_data: dict):
        return cls(
            ...
            agent_wallets=[GameAgentWallet.from_dict(w) for w in game_data.get("agent_wallets", [])],
...
class GameAPIClient:
    ...
    def request(method: str, endpoint: str, params=None, data=None, json=None):
        ...
        try:
            ...
            logger.info(f"[GameAPIClient.request] Successfully called {method} {url} with response: {response.json()}")
            return response.json(), None
        except requests.exceptions.RequestException as e:
            logger.error(f"[GameAPIClient.request] API call to {url} failed with error: {str(e)}")
    ...
    @staticmethod
    def get_game_info_by_tweet_id(tweet_id: str) -> tuple[GameInfo | None, Exception | None]:
        """
        ...
        if err is None and response_data.get("status") == 1:
            ...
            logger.info(f"[GameAPIClient.get_game_info_by_tweet_id] Failed to get game info for tweet {tweet_id}. Status: {response_data.get('status') if response_data else 'No response'}")
            return None, Exception(f"Failed to get game info for tweet {tweet_id}")


    @staticmethod
    def get_game_info_by_tweet_ids(tweet_ids: list[str]) -> tuple[list[GameInfo] | None, Exception | None]:
        """
        ...
        if err is None and response_data.get("status") == 1:
            ...
            game_infos = [GameInfo.from_dict(game_data) for game_data in games_data]
            logger.info(f"[GameAPIClient.get_game_info_by_tweet_ids] Successfully retrieved info for {len(game_infos)} games")
            return game_infos, None
        else:
            logger.info(f"[GameAPIClient.get_game_info_by_tweet_ids] Failed to get game info for tweets {tweet_ids}. Status: {response_data.get('status') if response_data else 'No response'}")
            return None, Exception(f"Failed to get game info for tweets {tweet_ids}")
    ...
    def end_game(tweet_id: str):
        ...
        if err is None and response_data.get("status") == 1:
            logger.info(f"[GameAPIClient.end_game] Successfully ended game for tweet {tweet_id}")
            return GameInfo.from_dict(response_data.get("data", {})), None
        else:
            logger.info(f"[GameAPIClient.end_game] Failed to end game for tweet {tweet_id}. Status: {response_data.get('status') if response_data else 'No response'}")
            return None, Exception(f"Failed to end game for tweet {tweet_id}")

    @staticmethod
    def start_game(tweet_id: str, agent_usernames: list, time_out: int) -> tuple[GameInfo | None, Exception | None]:
        """
        Starts a new game
        ...
        if err is None and response_data.get("status") == 1:
            logger.info(f"[GameAPIClient.start_game] Successfully started game for tweet {tweet_id} with agents {agent_usernames}")
            return GameInfo.from_dict(response_data.get("data", {})), None
        else:
            logger.info(f"[GameAPIClient.start_game] Failed to start game for tweet {tweet_id}. Status: {response_data.get('status') if response_data else 'No response'}")
            return None, Exception(f"Failed to start game for tweet {tweet_id}")
    ...
    def submit_game_result(tweet_id: str, agent_username: str):
        ...
        if err is None and response_data.get("status") == 1:
            logger.info(f"[GameAPIClient.submit_game_result] Successfully submitted game result for tweet {tweet_id}. Winner: {agent_username}")
            return GameInfo.from_dict(response_data.get("data", {})), None

        else:
            logger.info(f"[GameAPIClient.submit_game_result] Failed to submit game result for tweet {tweet_id}. Status: {response_data.get('status') if response_data else 'No response'}")
            return None, Exception(f"Failed to submit game result for tweet {tweet_id}")
