...
@log_function_call
async def search_from_db(kn_base: KnowledgeBase, query: str, top_k: int = 10, threshold = 0.8) -> List[str]:
    """
...
def get_random_from_collection(collection_name: str, n = 10):
    headers = {
        "X-Token": const.RAG_SECRET_TOKEN
    }
    ...
    if resp.status_code != 200:
        return [], Exception(f"Failed to get random vectors from collection {collection_name}")
...
@log_function_call
def insert_to_db(collection_name: str, contents: List[str], batch_size: int = 8):
    headers = {
        "X-Token": const.RAG_SECRET_TOKEN
    }
