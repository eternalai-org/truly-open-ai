...
def get_list_of_tradable_tokens(ttl: int = 0) -> List[Dict]:
    ...
    url = 'https://imagine-backend.bvm.network/api/internal/trade/tokens'
    ...
    if resp.status_code != 200:
        logger.info(f"Failed to get tradable symbols. Status code: {resp.status_code}.; {resp.text}")
        return []

    return resp.json()['result']

def get_trading_history(chain_id: int, agent_contract_id: str):
    url = f"https://agent.api.eternalai.org/api/internal/twitter/wallet/solana/trades/{chain_id}/{agent_contract_id}"
    ...
    if resp.status_code != 200:
        logger.error(f"Failed to get trading pnl. Status code: {resp.status_code}; Text: {resp.text}")
    ...
    for item in resp_json_result:
        mintaddr, side, amount_in, amount_out = item.get("mint"), item.get("side"), float(item.get("amount_in")), float(item.get("amount_out"))
        ...
        if price < 1e-4:
            resp_template["history"].append(("Bought" if side == "buy" else "Sold") + f" {amount:,.4f} ${symbol} at price {price:,.6f} SOL")
        else:
            resp_template["history"].append(("Bought" if side == "buy" else "Sold") + f" {amount:,.4f} ${symbol} at price {price:,.4f} SOL")

    return resp_template
    
def get_trading_pnl(chain_id: int, agent_contract_id: str):
    url = f'https://agent.api.eternalai.org/api/internal/twitter/wallet/solana/pnls/{chain_id}/{agent_contract_id}'
    headers = {
        'api-key': const.TWITTER_API_KEY
    }
    ...
    for mintaddr, pnl_data in resp_json_result.items():
        ...
        pnl_resp["pnl_in_SOL"][symbol] = float(f'{float(pnl_data["pnl_amount"]):.4f}')
...
def fetch_symbol2mintaddr(ttl: int = 0):
    ...
    symbol2mintaddr = {
        ...
        for v in tradable if v["token_symbol"] is not None and v["token_address"] is not None
    }
...
def get_market_news(ref_id: str, ttl=None) -> List[TweetObject]:
    ...
    url = 'https://imagine-backend.bvm.network/api/internal/twitter/tweets/by/agent'
    ...
    if resp.status_code != 200:
        logger.error(f"Failed to get market news. Status code: {resp.status_code}; Text: {resp.text}")
        random5tokens = random.sample(list_of_tradable_symbols, min(5, len(list_of_tradable_symbols)))
    ...
    tweets = [
        ...
        if any([symbol.lower() in x["text"].lower() 
    ...
    while len(tweets) < 10:
        ...
        random5tokens = random.sample(list_of_tradable_symbols, min(4, len(list_of_tradable_symbols)))
        resp = twitter_v2.search_for_token_news(random5tokens)
...
def get_base_token_price_in_usd(ttl_hash=None):
    ...
    resp = requests.get('https://agent.api.eternalai.org/api/coin-prices')
...
def _get_dexscreen_prices(symbol: str, ttl_hash=None):
    ...
    if mintaddress is None:
        ...
        logger.error(f"Invalid symbol {symbol}. Symbol must be one of {tradable_symbols_str}")
        return None
    ...
    resp = requests.get(
        f'https://agent.api.eternalai.org/api/internal/trade/dexscreen-info/', 
        headers=headers,
        params={
    ...
    if resp.status_code != 200:
        logger.error(f"Failed to get price for token ${symbol}. Status code: {resp.status_code}.")
        return None
...
def get_wallet_balance(chain_id: int, agent_contract_id: str, ttl_hash=None):
    ...
    wallet = {
        ...
        'balances': {
            ...
            **{
                k: 0
                for k in fetch_symbol2mintaddr(get_ttl_hash(300)).keys()
            }
        }
    }
...
# trading tools
def buy(chain_id: int, agent_contract_id: str, symbol: str, amount: str, ref: str, sol_limit=.1):
    ...
    if sol_amount > sol_limit:
        return trading_respond(f"The tradable amount is limited to {sol_limit} SOL, but you are trying to buy ${symbol} with total of {sol_amount} SOL which is not allowed.", False)
    ...
    if mint_addr is None:
        list_of_tradable_symbols = get_tradable_symbols()
        tradable_symbols_str = ', '.join(['$' + x for x in list_of_tradable_symbols])
        return trading_respond(f"Invalid symbol ${symbol}. Symbol must be one of {tradable_symbols_str}", False)
    
    _dex_result = _get_dexscreen_prices(symbol, ttl_hash=get_ttl_hash(300))

    if _dex_result is None:
        return trading_respond(f"Failed to get the market data for token ${symbol}", False)
    ...
    wallet_ballance = get_wallet_balance(chain_id, agent_contract_id, ttl_hash=get_ttl_hash(120))
    if not isinstance(wallet_ballance, dict):
        logger.error(f"Failed to check current wallet balance. {wallet_ballance}")
        return trading_respond(f"Failed to check current wallet balance.", False)

    sol_balance = wallet_ballance['balances']['SOL']

    if sol_balance < sol_amount + GAS_FEE_SOL:
        return trading_respond(f"Insufficient balance. You have {sol_balance} SOL, but need {sol_amount} SOL to buy {amount} ${symbol} and charge gas fee.", False)
...
def sell(chain_id: int, agent_contract_id: str, symbol: str, amount: str, ref_id: str, sol_limit=.5):
    symbol = symbol.upper().strip(' $')
    ...
    try:
        ...
        logger.error(f"Failed to parse the number {amount} due to {err}")
        return trading_respond(f"Amount must be a real number, {amount} is not", False)

    wallet_ballance = get_wallet_balance(chain_id, agent_contract_id)
    token_balance = wallet_ballance['balances'].get(symbol, 0)

    if token_balance < amount:
        return trading_respond(f"Insufficient balance. You have {token_balance} ${symbol}, but need {amount} ${symbol} to sell.", False)

    sol_balance = wallet_ballance['balances']['SOL']

    if sol_balance < GAS_FEE_SOL:
        return trading_respond(f"Insufficient balance. You have {sol_balance} SOL, but need {GAS_FEE_SOL} SOL to charge gas fee.", False)
    ...
    if is_mocking_action():
        return trading_respond(f"Sold {amount:,.4f} token ${symbol} and estimated to get {sol_back:,.4f} SOL back", True)
